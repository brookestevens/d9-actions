...
namespace Drupal\Tests\d9_test_profile\Functional;
...
/**
 * Tests d9_test_profile profile.
 *
 * @group d9_test_profile
 */
class D9TestProfileTest extends BrowserTestBase {
   ...
   * {@inheritdoc}
   */
  protected $profile = 'd9_test_profile';
  ...
  public function testConfig() {
    ...
    $default_config_storage = new FileStorage(drupal_get_path('profile', 'd9_test_profile') . '/' . InstallStorage::CONFIG_INSTALL_DIRECTORY, InstallStorage::DEFAULT_COLLECTION);
    $this->assertDefaultConfig($default_config_storage, $active_config_storage);

    $default_config_storage = new FileStorage(drupal_get_path('profile', 'd9_test_profile') . '/' . InstallStorage::CONFIG_OPTIONAL_DIRECTORY, InstallStorage::DEFAULT_COLLECTION);
    $this->assertDefaultConfig($default_config_storage, $active_config_storage);
  }
  ...
  public function testEditNodesByAdmin() {
    ...
    $nodes = $this->container->get('entity_type.manager')
      ->getStorage('node')
      ->loadByProperties(['title' => 'Nextpress Demo Page']);
    $node = reset($nodes);
    $this->drupalGet($node->toUrl('edit-form'));
    $webassert->statusCodeEquals('200');
    $this->submitForm([], "Save");
    $webassert->pageTextContains('Nextpress Demo Page has been updated.');
  }
  ...
  public function testAppearance() {
    ...
    $webassert->pageTextContains('CU Boulder 2019 Base Theme');
  }
